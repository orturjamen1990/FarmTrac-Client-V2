...
import GrowerEditorDialog from "./components/GrowerEditorDialog";
import { Button } from "@/components/ui/button";
import Lucide from "@/components/Base/Lucide";
import growerService from "./services/growerService";
import { ColumnDef, createColumnHelper } from "@tanstack/react-table";
import { useTranslation } from "react-i18next";
import { GrowerRes } from "./types/types";
...
const Grower = () => {
  ...
  const [growers, setGrowers] = useState<GrowerRes[]>([]);
  const [grower, setGrower] = useState<GrowerRes>();
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.id")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.name")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.email")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.establishedYear")}
        />
      ),
      cell: ({ row }) => <div>{row.getValue("establishedYear")}</div>,
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.location")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.contactNumber")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("growers:table.columns.active")}
        />
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("customerType:fields.notes")}
        />
      ),
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent className="pt-[12px] pb-[12px]" align="end">
              <DropdownMenuItem
                onClick={() => onEditGrower(grower.id)}
  ...
  const fetchData = async () => {
    const { data } = await growerService.getGrowers();
    setGrowers(data.data);
  };

  const onEditGrower = async (id: number) => {
    const { data } = await growerService.getGrowerById(id);
    setGrower(data.data);
    setIsOpen(true);
  };
  ...
  const onDelete = async (row: any) => {
    confirm({
      ...
      onConfirm: async () => {
        const response = await growerService.deleteGrower(row.id);
        await fetchData();
