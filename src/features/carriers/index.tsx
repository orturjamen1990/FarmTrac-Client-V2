...
import CustomerEditorDialog from "./components/CarrierEditorDialog";
...
import { CarrierRes } from "./types";
import { Badge } from "@/components/ui/badge";
import CarrierEditorDialog from "./components/CarrierEditorDialog";

const Customers = () => {
  const { t } = useTranslation(["global", "carriers"]);
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [carriers, setCarriers] = useState<CarrierRes[]>([]);
  const [carrier, setCarrier] = useState<CarrierRes>();
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carriers:table.columns.name")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carriers:table.columns.contactEmail")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carriers:table.columns.contactPhone")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carriers:table.columns.address")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("carriers:table.columns.notes")}
        />
      ),
  ...
  const onDelete = async (row: any) => {
    confirm({
      ...
      onConfirm: async () => {
        const response = await carrierService.deleteCarrier(row.id);
        await fetchData();
