...
const ProduceSpeciesEditorDialog = ({
  isOpen,
  onClose,
  item,
  refetch,
  ...
  type ProduceSpecies = z.infer<typeof formSchema>;
  ...
  const form = useForm<ProduceSpecies>({
    resolver: zodResolver(formSchema),
    defaultValues,
  });

  const onSubmit = async (values: ProduceSpecies) => {
    let response;
    try {
      if (values.id) {
        const data = values as any;
        response = await produceService.updateProduceSpecies(data.id, data);
      } else {
        const data = values as any;
        response = await produceService.createProduceSpecies(data);
      }
      await refetch?.();
      onClose();
    ...
    if (isOpen) {
      setTitle(
        !item?.id
          ...
          : `${t("update")} ${t("produceSpecies:title")}`
      );
    }
  }, [item, isOpen]);
  ...
  useEffect(() => {
    if (item) {
      form.reset(item as ProduceSpecies); // Reset the form with the item values
    }
  }, [item, form]);
              ...
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <>
                      <FormItem>
                        <FormLabel>
                          {t("produceSpecies:form.produceId.label")}
                        </FormLabel>
                        <FormControl>
                          <Combobox
                            value={field.value}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("produceSpecies:form.name.label")}
                      </FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={t(
                            "produceSpecies:form.name.placeholder"
                          )}
                        />
                      </FormControl>
                      <FormMessage />
...
export default ProduceSpeciesEditorDialog;
