...
import ProduceEditorDialog from "./components/ProduceEditorDialog";

const Produces = () => {
  const { t } = useTranslation(["global", "produce"]);
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [produces, setProduces] = useState<ProduceRes[]>([]);
      ...
      cell: ({ row }) => (
        <div>
          <Link
            ...
            to={"/produces/$produceId"}
            params={{
              produceId: Number(row.getValue("id")),
            }}
          >
            {row.getValue("id")}
          </Link>
  ...
  const fetchData = async () => {
    const response = await produceService.getProduces();
    setProduces(response.data.data);
  };

  const onEdit = async (id: number) => {
    const { data } = await produceService.getProduceById(id);
  ...
  const onDelete = async (row: any) => {
    confirm({
      ...
      onConfirm: async () => {
        const response = await produceService.deleteProduce(row.id);
        await fetchData();
