...
import CustomerTypeEditorDialog from "./components/customer-type-editor-dialog";
...
import { CustomerTypeRes } from "./types/types";
...
const CustomerType = () => {
  ...
  const [customerTypes, setCustomerTypes] = useState<CustomerTypeRes[]>([]);
  const [customerType, setCustomerType] = useState<CustomerTypeRes>();
  const columnHelper = createColumnHelper<CustomerTypeRes>();
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("customerType:fields.id")}
        />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("customerType:fields.name")}
        />
      ),
    ...
    },
    {
      ...
      minSize: 80,
      cell: ({ row }) => {
        const cusomerType = row.original;
  ...
  const onEdit = async (id: number) => {
    const { data } = await customerTypeService.getCustomerTypeById(id);
  ...
  const onDelete = async (row: any) => {
    confirm({
      ...
      onConfirm: async () => {
        const response = await customerTypeService.deleteCustomerType(row.id);
        await fetchData();
